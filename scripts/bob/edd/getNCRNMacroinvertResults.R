...
options(warn=-1)
getNCRNMacroinvertResults <- function(results_list, example){
    tryCatch(
        expr = {
            ...
            real[2] <- df$Benthic_Data_ID # "Activity_ID" shared field with `real_activities.Activity_ID`
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`getNCRNMacroinvertResults()` executed successfully..."
                } else {
