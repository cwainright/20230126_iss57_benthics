...
options(warn=-1)
getBob2022HabActivities <- function(results_list, bob_2022_hab, example){
  tryCatch(
    expr = {
      ...
      real[4] <- paste0(df$Site, "_", format(df$`Date sampled`, "%Y%m%d"), "_benthic_habitat_inventory") # "Activity_ID" shared field with `real_locations.Activity_ID` and `real_results.Activity_ID`
      ...
      message(
        if(length(check_df$result == "MATCH") == nrow(check_df)){
          "`getBob2022HabActivities()` executed successfully..."
        } else {
