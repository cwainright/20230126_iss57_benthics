...
options(warn=-1)
getBob2022HabResultsUnitLookup <- function(){
  tryCatch(
    expr = {
      ...
      unit_lookup <- readxl::read_excel("data/unit_lookup.xlsx") # no longer need to make our own lu table, just reference Bob's...
      
      message("`getBob2022HabResultsUnitLookup()` executed successfully...")
      return(unit_lookup)
    }
  )
}