...
getEDDLocations <- function(results_list, habitat_marc2021, habitat_marc2022, bob_2021_macroinvert, bob_2021_water_chem, addBob){
    tryCatch(
        expr = {
            ...
            source("scripts/edd/getNCRNMacroinvertLocations.R")
            ...
            real <- rbind(ncrn_fish_locations, ncrn_chem_locations, ncrn_hab_locations, marc_habitat_locations) # row bind project function returns
            
            #----- if TRUE add Marc's 2022 data to dataframe `real` NCRN db data
            if(addBob==TRUE){
              bob_2021_macroinvert_locations <- getBob2021MacroinvertsLocations()
              bob_2021_chem_locations <- getBob2021ChemLocations()
              # combine
