...
options(warn=-1)
getNCRNMacroinvertActivities <- function(results_list, example){
    tryCatch(
        expr = {
            ...
            real[4] <- df$Benthic_Data_ID # "Activity_ID" shared field with `real_locations.Activity_ID` and `real_results.Activity_ID`
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`getNCRNMacroinvertActivities()` executed successfully..."
                } else {
