...
buildMarc2021Indiv <- function(example, marc2021, tlu_species){
    tryCatch(
        expr = {
            #--------------------------------------------------------------------------
            ...
            indiv[15] <- df$Disposition # "Disposition"
            indiv[16] <- df$Picture # "Picture"  
            indiv[17] <- df$Camera # "Camera" 
            indiv[18] <- df$Photo # "Photo"
            ...
            indiv[32] <- '"data/NCRN_BSS_Fish_Monitoring_Data_2021_Marc.xlsx", sheet = "Fish Data (Individuals)"'
