...
options(warn=-1)
getNCRNChemResults <- function(results_list, example){
    tryCatch(
        expr = {
            ...
            real[2] <- df$Chem_Data_ID # "Activity_ID" shared field with `real_activities.Activity_ID`
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`getNCRNChemResults()` executed successfully..."
                } else {
